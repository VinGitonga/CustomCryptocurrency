from mutuall.models import User
from mutuall.forms import *
from flask import Flask, jsonify, request, render_template, url_for, flash, redirect
from mutuall import app, db, bcrypt
from mutuall import blockchainObj
...
def transaction():
    ...
    if form.validate_on_submit():
        ...
        blockchainObj.addMutualAidTransaction(0.2 * form.amount.data, form.key.data, form.key.data)
        if feedback:
...
@app.route("/account/donations")
@login_required
def accountDonations():
    return render_template('accountDonations.html', title='Donations', blockchain = blockchainObj)
...
'''
Mine mutual aid transactions
'''
@app.route('/mine_mutual', methods=['GET'])
def mineMutualAId():
    print('Made it to Mine Mutual Aid')
    lastBlock = blockchainObj.getLastBlock(isMutual=True);

    if len(blockchainObj.mutualTransactions) < 1:
        flash(f'Not enough mutual transactions to mine! (Must be > 1)', 'danger');
    else:
        feedback = blockchainObj.mineMutualTransactions()
        if feedback:
            flash(f'Block Mined! Participants will receive some donations!', 'success');
        else:
            flash(f'Error!', 'danger');
    return render_template('mutual_mine.html', title = "Mutual", blockchain = blockchainObj)
...
@app.route('/transactions/new/mutual', methods=['POST'])
def new_transaction_mutual():
    values = request.get_json();
    required = ['amt']
    if not all(k in values for k in required):
        return 'Missing values', 400;

    index = blockchainObj.addMutualAidTransaction(0.02*values['amt'],values['sender'], values['reciever'])

    response = {'message': f'Mutual Aid Transaction will be added to Block {index}'}
    return jsonify(response), 201;
...
@app.route('/chain/mutual', methods=['GET'])
def full_chain_mutual():
    response = {
        'chain': blockchainObj.chainJSONencode(isMutual=True),
        'length': len(blockchainObj.chain),
    }
    return jsonify(response), 200
...
@app.route('/nodes/resolve/mutual', methods=['GET'])
def consensusMutual():
    replaced = blockchainObj.resolveConflicts(isMutual=True)

    if replaced:
        response = {
            'message': 'Our chain was replaced',
            'new_chain': blockchainObj.chainJSONencode(isMutual=True)
        }
    else:
        response = {
            'message': 'Our chain is authoritative',
